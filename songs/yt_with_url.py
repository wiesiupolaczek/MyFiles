...
class YouTubeDownloaderApp:
    def __init__(self):
        ...
        self.app.geometry("800x600")  # Increased size to accommodate sidebar
    ...
    def create_sidebar(self):
        ...
        # Optional: Add a logo or image at the bottom
        # self.logo = ctk.CTkImage(light_image=..., dark_image=..., size=(...))
        # self.logo_label = ctk.CTkLabel(self.sidebar, image=self.logo)
        # self.logo_label.grid(row=3, column=0, padx=20, pady=20)
    ...
    def create_youtube_frame(self, frame):
        ...
        url_label = ctk.CTkLabel(frame, text="Enter the YouTube video or playlist URL:")
        ...
        self.youtube_download_button = download_button
        self.youtube_status_label = status_label

    def authenticate_spotify(self):
        """
        Authenticate with Spotify and load user playlists.
        """
        try:
            playlists = []
            ...
            self.spotify_playlist_dropdown.configure(values=playlist_names, state="normal")
    ...
    def download_tracks(self, tracks):
        ...
        for idx, item in enumerate(tracks):
            ...
            # No need to call self.app.update() when using queue
    ...
    def download_youtube_from_url(self, url):
        ...
        """
        if not url.strip():
            self.youtube_status_label.configure(text="Please enter a YouTube URL.")
            return

        self.youtube_status_label.configure(text=f"Starting download: {url}")
        logging.info(f"Starting download for YouTube URL: {url}")

        # Start download in a separate thread
        download_thread = threading.Thread(target=self.handle_youtube_download, args=(url,), daemon=True)
        download_thread.start()
                ...
                if 'entries' in info_dict:
                    ...
                    for entry in info_dict['entries']:
                        if entry:  # Some entries might be None
                            ...
                            self.queue.put(
                                (self.youtube_status_label.configure, ({"text": f"Downloaded: {audio_title}"})))
                            logging.info(f"Downloaded and converted to MP3: {audio_title}")
                else:
                    # It's a single video
                    audio_title = sanitize_filename(info_dict['title']) + '.mp3'
                    self.queue.put((self.youtube_status_label.configure, ({"text": f"Downloaded: {audio_title}"})))
                    logging.info(f"Downloaded and converted to MP3: {audio_title}")

        except Exception as e:
            self.queue.put((self.youtube_status_label.configure, ({"text": f"Error downloading: {url}"})))
    ...
    def reset_app(self):
        ...
        self.spotify_playlist_var.set('')
        self.spotify_status_label.configure(text="")
        self.youtube_url_entry.delete(0, tk.END)
        self.youtube_status_label.configure(text="")
        self.spotify_download_button.configure(state="disabled")
        self.youtube_download_button.configure(state="normal")  # Ensure YouTube download is enabled
        self.app.update()
